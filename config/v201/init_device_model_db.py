...
DATABASE_NAME = "device_model_ocpp201.db"
...
def insert_variable(variable: dict, component_id: str, variable_characteristics_id: int, cur: sqlite3.Cursor):
    ...
    Args:
        ...
        cur (sqlite3.Cursor): sqlite3 cursor
    """
    data = (variable, component_id, variable_characteristics_id)
    cur.execute(
        f"INSERT OR REPLACE INTO VARIABLE (NAME, COMPONENT_ID, VARIABLE_CHARACTERISTICS_ID) VALUES(?, ?, ?)", data)
...
def insert_component(con: sqlite3.Connection, component_schema: dict, component_name: str):
    ...
    for variable, variable_meta_data in component_schema.get("properties").items():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--out", metavar='OUT',
                        help="Path to where the directory where the database file should be located", required=True)
    ...
    con = sqlite3.connect(Path(out_file).joinpath(DATABASE_NAME))
    execute_init_sql(con)
    print("Initialized the database")
    print("Starting to insert components and variables")
